...
inductive partrec (O : ℕ → ℕ) : (ℕ →. ℕ) → Prop
| zero : partrec (pure 0)
| succ : partrec succ
...
theorem of_eq {O : ℕ → ℕ} {f g : ℕ →. ℕ} (hf : partrec O f) (H : ∀ n, f n = g n) : partrec O g :=
(funext H : f = g) ▸ hf

theorem of_eq_tot {O : ℕ → ℕ} {f : ℕ →. ℕ} {g : ℕ → ℕ}
  (hf : partrec O f) (H : ∀ n, g n ∈ f n) : partrec O g :=
hf.of_eq (λ n, eq_some_iff.2 (H n))

theorem of_primrec {O : ℕ → ℕ} {f : ℕ → ℕ} (hf : primrec f) : partrec O f :=
begin
  induction hf,
...
protected theorem some {O : ℕ → ℕ} : partrec O some := of_primrec primrec.id

theorem none {O : ℕ → ℕ} : partrec O (λ n, none) :=
(of_primrec (nat.primrec.const 1)).rfind.of_eq $
λ n, eq_none_iff.2 $ λ a ⟨h, e⟩, by simpa using h

theorem prec' {O : ℕ → ℕ} {f g h}
  (hf : partrec O f) (hg : partrec O g) (hh : partrec O h) :
  partrec O (λ a, (f a).bind (λ n, n.elim (g a)
...
theorem ppred {O : ℕ → ℕ} : partrec O (λ n, ppred n) :=
...
def partrec {α σ} [primcodable α] [primcodable σ] (O : ℕ → ℕ)
  (f : α →. σ) := nat.partrec O (λ n,
  roption.bind (decode α n) (λ a, (f a).map encode))

def partrec₂ {α β σ} [primcodable α] [primcodable β] [primcodable σ]
  (O : ℕ → ℕ) (f : α → β →. σ) := partrec O (λ p : α × β, f p.1 p.2)

def computable {α σ} [primcodable α] [primcodable σ] (O : ℕ → ℕ) (f : α → σ) := partrec O (f : α →. σ)

def computable₂ {α β σ} [primcodable α] [primcodable β] [primcodable σ]
  (O : ℕ → ℕ) (f : α → β → σ) := computable O (λ p : α × β, f p.1 p.2)

theorem primrec.to_comp {α σ} [primcodable α] [primcodable σ]
  {O : ℕ → ℕ} {f : α → σ} (hf : primrec f) : computable O f :=
...
theorem primrec₂.to_comp {α β σ} [primcodable α] [primcodable β] [primcodable σ]
  {O : ℕ → ℕ} {f : α → β → σ} (hf : primrec₂ f) : computable₂ O f := hf.to_comp

theorem computable.part {α σ} [primcodable α] [primcodable σ]
  {O : ℕ → ℕ} {f : α → σ} (hf : computable O f) : partrec O (f : α →. σ) := hf

theorem computable₂.part {α β σ} [primcodable α] [primcodable β] [primcodable σ]
  {O : ℕ → ℕ} {f : α → β → σ} (hf : computable₂ O f) : partrec₂ O (λ a, (f a : β →. σ)) := hf

namespace computable
variables {α : Type*} {β : Type*} {γ : Type*} {σ : Type*}
variables [primcodable α] [primcodable β] [primcodable γ] [primcodable σ]

theorem of_eq {O : ℕ → ℕ} {f g : α → σ} (hf : computable O f) (H : ∀ n, f n = g n) : computable O g :=
(funext H : f = g) ▸ hf

theorem const (s : σ) {O : ℕ → ℕ} : computable O (λ a : α, s) :=
(primrec.const _).to_comp

theorem of_option {O : ℕ → ℕ} {f : α → option β}
  (hf : computable O f) : partrec O (λ a, (f a : roption β)) :=
(nat.partrec.ppred.comp hf).of_eq $ λ n, begin
  cases decode α n with a; simp,
  cases f a with b; simp
end

theorem to₂ {O : ℕ → ℕ} {f : α × β → σ} (hf : computable O f) : computable₂ O (λ a b, f (a, b)) :=
hf.of_eq $ λ ⟨a, b⟩, rfl

protected theorem id {O : ℕ → ℕ} : computable O (@id α) := primrec.id.to_comp

theorem fst {O : ℕ → ℕ} : computable O (@prod.fst α β) := primrec.fst.to_comp

theorem snd {O : ℕ → ℕ} : computable O (@prod.snd α β) := primrec.snd.to_comp

theorem pair {O : ℕ → ℕ} {f : α → β} {g : α → γ}
  (hf : computable O f) (hg : computable O g) : computable O (λ a, (f a, g a)) :=
(hf.pair hg).of_eq $
λ n, by cases decode α n; simp [(<*>)]

theorem unpair {O : ℕ → ℕ} : computable O nat.unpair := primrec.unpair.to_comp

theorem succ {O : ℕ → ℕ} : computable O nat.succ := primrec.succ.to_comp
theorem pred {O : ℕ → ℕ} : computable O nat.pred := primrec.pred.to_comp
theorem nat_bodd {O : ℕ → ℕ} : computable O nat.bodd := primrec.nat_bodd.to_comp
theorem nat_div2 {O : ℕ → ℕ} : computable O nat.div2 := primrec.nat_div2.to_comp

theorem sum_inl {O : ℕ → ℕ} : computable O (@sum.inl α β) := primrec.sum_inl.to_comp
theorem sum_inr {O : ℕ → ℕ} : computable O (@sum.inr α β) := primrec.sum_inr.to_comp

theorem list_cons {O : ℕ → ℕ} : computable₂ O (@list.cons α) := primrec.list_cons.to_comp
theorem list_reverse {O : ℕ → ℕ} : computable O (@list.reverse α) := primrec.list_reverse.to_comp
theorem list_nth {O : ℕ → ℕ} : computable₂ O (@list.nth α) := primrec.list_nth.to_comp
theorem list_append {O : ℕ → ℕ} : computable₂ O ((++) : list α → list α → list α) := primrec.list_append.to_comp
theorem list_concat {O : ℕ → ℕ} : computable₂ O (λ l (a:α), l ++ [a]) := primrec.list_concat.to_comp
theorem list_length {O : ℕ → ℕ} : computable O (@list.length α) := primrec.list_length.to_comp

theorem vector_cons {n} {O : ℕ → ℕ} : computable₂ O (@vector.cons α n) := primrec.vector_cons.to_comp
theorem vector_to_list {n} {O : ℕ → ℕ} : computable O (@vector.to_list α n) := primrec.vector_to_list.to_comp
theorem vector_length {n} {O : ℕ → ℕ} : computable O (@vector.length α n) := primrec.vector_length.to_comp
theorem vector_head {n} {O : ℕ → ℕ} : computable O (@vector.head α n) := primrec.vector_head.to_comp
theorem vector_tail {n} {O : ℕ → ℕ} : computable O (@vector.tail α n) := primrec.vector_tail.to_comp
theorem vector_nth {n} {O : ℕ → ℕ} : computable₂ O (@vector.nth α n) := primrec.vector_nth.to_comp
theorem vector_nth' {n} {O : ℕ → ℕ} : computable O (@vector.nth α n) := primrec.vector_nth'.to_comp
theorem vector_of_fn' {n} {O : ℕ → ℕ} : computable O (@vector.of_fn α n) := primrec.vector_of_fn'.to_comp

theorem fin_app {n} {O : ℕ → ℕ} : computable₂ O (@id (fin n → σ)) := primrec.fin_app.to_comp

protected theorem encode {O : ℕ → ℕ} : computable O (@encode α _) :=
primrec.encode.to_comp

protected theorem decode {O : ℕ → ℕ} : computable O (decode α) :=
primrec.decode.to_comp

protected theorem of_nat (α) [denumerable α] {O : ℕ → ℕ} : computable O (of_nat α) :=
(primrec.of_nat _).to_comp

theorem encode_iff {O : ℕ → ℕ} {f : α → σ} : computable O (λ a, encode (f a)) ↔ computable O f :=
iff.rfl

theorem option_some {O : ℕ → ℕ} : computable O (@option.some α) :=
...
theorem of_eq {O : ℕ → ℕ} {f g : α →. σ} (hf : partrec O f) (H : ∀ n, f n = g n) : partrec O g :=
(funext H : f = g) ▸ hf

theorem of_eq_tot {O : ℕ → ℕ} {f : α →. σ} {g : α → σ}
  (hf : partrec O f) (H : ∀ n, g n ∈ f n) : computable O g :=
hf.of_eq (λ a, eq_some_iff.2 (H a))

theorem none {O : ℕ → ℕ} : partrec O (λ a : α, @roption.none σ) :=
nat.partrec.none.of_eq $ λ n, by cases decode α n; simp

protected theorem some {O : ℕ → ℕ} : partrec O (@roption.some α) := computable.id

theorem const' (s : roption σ) {O : ℕ → ℕ} : partrec O (λ a : α, s) :=
by haveI := classical.dec s.dom; exact
(of_option (const (to_option s))).of_eq (λ a, of_to_option s)

protected theorem bind {O : ℕ → ℕ} {f : α →. β} {g : α → β →. σ}
  (hf : partrec O f) (hg : partrec₂ O g) : partrec O (λ a, (f a).bind (g a)) :=
(hg.comp (nat.partrec.some.pair hf)).of_eq $
λ n, by simp [(<*>)]; cases e : decode α n with a;
  simp [e, encodek]

theorem map (O : ℕ → ℕ) {f : α →. β} {g : α → β → σ}
  (hf : partrec O f) (hg : computable₂ O g) : partrec O (λ a, (f a).map (g a)) :=
by simpa [bind_some_eq_map] using
   @@partrec.bind _ _ _ _ (λ a b, roption.some (g a b)) hf hg

theorem to₂ {O : ℕ → ℕ} {f : α × β →. σ} (hf : partrec O f) : partrec₂ O (λ a b, f (a, b)) :=
hf.of_eq $ λ ⟨a, b⟩, rfl

theorem nat_elim
  {O : ℕ → ℕ} {f : α → ℕ} {g : α →. σ} {h : α → ℕ × σ →. σ}
  (hf : computable O f) (hg : partrec O g) (hh : partrec₂ O h) :
  partrec O (λ a, (f a).elim (g a) (λ y IH, IH.bind (λ i, h a (y, i)))) :=
...
theorem comp {O : ℕ → ℕ} {f : β →. σ} {g : α → β}
  (hf : partrec O f) (hg : computable O g) : partrec O (λ a, f (g a)) :=
(hf.comp hg).of_eq $
λ n, by simp; cases e : decode α n with a;
  simp [e, encodek]

theorem nat_iff {O : ℕ → ℕ} {f : ℕ →. ℕ} : partrec O f ↔ nat.partrec O f :=
by simp [partrec, map_id']

theorem map_encode_iff {O : ℕ → ℕ} {f : α →. σ} : partrec O (λ a, (f a).map encode) ↔ partrec O f :=
iff.rfl
...
theorem unpaired {O : ℕ → ℕ} {f : ℕ → ℕ →. α} : partrec O (nat.unpaired f) ↔ partrec₂ O f :=
⟨λ h, by simpa using h.comp primrec₂.mkpair.to_comp,
 λ h, h.comp primrec.unpair.to_comp⟩

theorem unpaired' {O : ℕ → ℕ} {f : ℕ → ℕ →. ℕ} : nat.partrec O (nat.unpaired f) ↔ partrec₂ O f :=
partrec.nat_iff.symm.trans unpaired

theorem comp
  {O : ℕ → ℕ} {f : β → γ →. σ} {g : α → β} {h : α → γ}
  (hf : partrec₂ O f) (hg : computable O g) (hh : computable O h) :
  partrec O (λ a, f (g a) (h a)) := hf.comp (hg.pair hh)

theorem comp₂
  {O : ℕ → ℕ} {f : γ → δ →. σ} {g : α → β → γ} {h : α → β → δ}
  (hf : partrec₂ O f) (hg : computable₂ O g) (hh : computable₂ O h) :
  partrec₂ O (λ a b, f (g a b) (h a b)) := hf.comp hg hh
...
theorem comp {O : ℕ → ℕ} {f : β → σ} {g : α → β}
  (hf : computable O f) (hg : computable O g) :
  computable O (λ a, f (g a)) := hf.comp hg

theorem comp₂ {O : ℕ → ℕ} {f : γ → σ} {g : α → β → γ}
  (hf : computable O f) (hg : computable₂ O g) :
  computable₂ O (λ a b, f (g a b)) := hf.comp hg
...
theorem comp
  {O : ℕ → ℕ} {f : β → γ → σ} {g : α → β} {h : α → γ}
  (hf : computable₂ O f) (hg : computable O g) (hh : computable O h) :
  computable O (λ a, f (g a) (h a)) := hf.comp (hg.pair hh)

theorem comp₂
  {O : ℕ → ℕ} {f : γ → δ → σ} {g : α → β → γ} {h : α → β → δ}
  (hf : computable₂ O f) (hg : computable₂ O g) (hh : computable₂ O h) :
  computable₂ O (λ a b, f (g a b) (h a b)) := hf.comp hg hh
...
theorem rfind {O : ℕ → ℕ} {p : α → ℕ →. bool} (hp : partrec₂ O p) :
  partrec O (λ a, nat.rfind (p a)) :=
(nat.partrec.rfind $ hp.map O
...
theorem rfind_opt {O : ℕ → ℕ} {f : α → ℕ → option σ} (hf : computable₂ O f) :
  partrec O (λ a, nat.rfind_opt (f a)) :=
(rfind (primrec.option_is_some.to_comp.comp hf).part.to₂).bind
  (of_option hf)

theorem nat_cases_right
  {O : ℕ → ℕ} {f : α → ℕ} {g : α → σ} {h : α → ℕ →. σ}
  (hf : computable O f) (hg : computable O g) (hh : partrec₂ O h) :
  partrec O (λ a, (f a).cases (some (g a)) (h a)) :=
...
theorem bind_decode2_iff {O : ℕ → ℕ} {f : α →. σ} : partrec O f ↔
  nat.partrec O (λ n, roption.bind (decode2 α n) (λ a, (f a).map encode)) :=
⟨λ hf, nat_iff.1 $ (of_option primrec.decode2.to_comp).bind $
  (map O hf (computable.encode.comp snd).to₂).comp snd,
λ h, map_encode_iff.1 $ by simpa [encodek2]
  using (nat_iff.2 h).comp (@computable.encode α _ _)⟩

theorem vector_m_of_fn {O : ℕ → ℕ} : ∀ {n} {f : fin n → α →. σ}, (∀ i, partrec O (f i)) →
  partrec O (λ (a : α), vector.m_of_fn (λ i, f i a))
| 0     f hf := const _
...
theorem option_some_iff {O : ℕ → ℕ} {f : α → σ} : computable O (λ a, some (f a)) ↔ computable O f :=
⟨λ h, encode_iff.1 $ primrec.pred.to_comp.comp $ encode_iff.2 h,
 option_some.comp⟩

theorem bind_decode_iff {O : ℕ → ℕ} {f : α → β → option σ} : computable₂ O (λ a n,
  (decode β n).bind (f a)) ↔ computable₂ O f :=
...
λ hf, begin
  have : partrec O (λ a : α × ℕ, (encode (decode β a.2)).cases
    (some option.none) (λ n, roption.map (f a.1) (decode β n))) :=
  partrec.nat_cases_right (primrec.encdec.to_comp.comp snd)
    (const none) ((of_option (computable.decode.comp snd)).map O
      (hf.comp (fst.comp $ fst.comp fst) snd).to₂),
  refine this.of_eq (λ a, _),
  simp, cases decode β a.2; simp [encodek]
end⟩

theorem map_decode_iff {O : ℕ → ℕ} {f : α → β → σ} : computable₂ O (λ a n,
  (decode β n).map (f a)) ↔ computable₂ O f :=
bind_decode_iff.trans option_some_iff

theorem nat_elim
  {O : ℕ → ℕ} {f : α → ℕ} {g : α → σ} {h : α → ℕ × σ → σ}
  (hf : computable O f) (hg : computable O g) (hh : computable₂ O h) :
  computable O (λ a, (f a).elim (g a) (λ y IH, h a (y, IH))) :=
(partrec.nat_elim hf hg hh.part).of_eq $
λ a, by simp; induction f a; simp *

theorem nat_cases {O : ℕ → ℕ} {f : α → ℕ} {g : α → σ} {h : α → ℕ → σ}
  (hf : computable O f) (hg : computable O g) (hh : computable₂ O h) :
  computable O (λ a, (f a).cases (g a) (h a)) :=
nat_elim hf hg (hh.comp fst $ fst.comp snd).to₂

theorem cond {O : ℕ → ℕ} {c : α → bool} {f : α → σ} {g : α → σ}
  (hc : computable O c) (hf : computable O f) (hg : computable O g) :
  computable O (λ a, cond (c a) (f a) (g a)) :=
(nat_cases (encode_iff.2 hc) hg (hf.comp fst).to₂).of_eq $
λ a, by cases c a; refl

theorem option_cases {O : ℕ → ℕ} {o : α → option β} {f : α → σ} {g : α → β → σ}
  (ho : computable O o) (hf : computable O f) (hg : computable₂ O g) :
  @computable  _ σ _ _ O (λ a, option.cases_on (o a) (f a) (g a)) :=
option_some_iff.1 $
(nat_cases (encode_iff.2 ho) (option_some_iff.2 hf)
    (map_decode_iff.2 hg)).of_eq $
λ a, by cases o a; simp [encodek]; refl

theorem option_bind {O : ℕ → ℕ} {f : α → option β} {g : α → β → option σ}
  (hf : computable O f) (hg : computable₂ O g) :
  computable O (λ a, (f a).bind (g a)) :=
(option_cases hf (const option.none) hg).of_eq $
λ a, by cases f a; refl

theorem option_map {O : ℕ → ℕ} {f : α → option β} {g : α → β → σ}
  (hf : computable O f) (hg : computable₂ O g) : computable O (λ a, (f a).map (g a)) :=
option_bind hf (option_some.comp₂ hg)

theorem option_get_or_else {O : ℕ → ℕ} {f : α → option β} {g : α → β}
  (hf : computable O f) (hg : computable O g) :
  computable O (λ a, (f a).get_or_else (g a)) :=
(computable.option_cases hf hg (show computable₂ O (λ a b, b), from computable.snd)).of_eq $
λ a, by cases f a; refl

theorem subtype_mk {O : ℕ → ℕ} {f : α → β} {p : β → Prop} [decidable_pred p] {h : ∀ a, p (f a)}
    (hp : primrec_pred p) (hf : computable O f) :
  @computable _ _ _  (primcodable.subtype hp) O (λ a, (⟨f a, h a⟩ : subtype p)) :=
nat.partrec.of_eq hf $ λ n, by cases decode α n; simp [subtype.encode_eq]

theorem sum_cases
  {O : ℕ → ℕ} {f : α → β ⊕ γ} {g : α → β → σ} {h : α → γ → σ}
  (hf : computable O f) (hg : computable₂ O g) (hh : computable₂ O h) :
  @computable _ σ _ _ O (λ a, sum.cases_on (f a) (g a) (h a)) :=
option_some_iff.1 $
...
theorem nat_strong_rec
  {O : ℕ → ℕ} (f : α → ℕ → σ) {g : α → list σ → option σ} (hg : computable₂ O g)
  (H : ∀ a n, g a ((list.range n).map (f a)) = some (f a n)) : computable₂ O f :=
suffices computable₂ O (λ a n, (list.range n).map (f a)), from
  option_some_iff.1 $
...
theorem list_of_fn {O : ℕ → ℕ} : ∀ {n} {f : fin n → α → σ},
  (∀ i, computable O (f i)) → computable O (λ a, list.of_fn (λ i, f i a))
| 0     f hf := const []
| (n+1) f hf := by simp [list.of_fn_succ]; exact
  list_cons.comp (hf 0) (list_of_fn (λ i, hf i.succ))

theorem vector_of_fn {n} {O : ℕ → ℕ} {f : fin n → α → σ}
  (hf : ∀ i, computable O (f i)) : computable O (λ a, vector.of_fn (λ i, f i a)) :=
...
theorem option_some_iff {O : ℕ → ℕ} {f : α →. σ} :
  partrec O (λ a, (f a).map option.some) ↔ partrec O f :=
⟨λ h, (nat.partrec.ppred.comp h).of_eq $
   λ n, by simp [roption.bind_assoc, bind_some_eq_map],
 λ hf, hf.map O (option_some.comp snd).to₂⟩

theorem option_cases_right {O : ℕ → ℕ} {o : α → option β} {f : α → σ} {g : α → β →. σ}
  (ho : computable O o) (hf : computable O f) (hg : partrec₂ O g) :
  @partrec _ σ _ _ O (λ a, option.cases_on (o a) (some (f a)) (g a)) :=
have partrec O (λ (a : α), nat.cases (roption.some (f a))
  (λ n, roption.bind (decode β n) (g a)) (encode (o a))) :=
nat_cases_right (encode_iff.2 ho) hf.part $
  ((@computable.decode β _ _).comp snd).of_option.bind
...
theorem sum_cases_right
  {O : ℕ → ℕ} {f : α → β ⊕ γ} {g : α → β → σ} {h : α → γ →. σ}
  (hf : computable O f) (hg : computable₂ O g) (hh : partrec₂ O h) :
  @partrec _ σ _ _ O (λ a, sum.cases_on (f a) (λ b, some (g a b)) (h a)) :=
have partrec O (λ a, (option.cases_on
...
theorem sum_cases_left
  {O : ℕ → ℕ} {f : α → β ⊕ γ} {g : α → β →. σ} {h : α → γ → σ}
  (hf : computable O f) (hg : partrec₂ O g) (hh : computable₂ O h) :
  @partrec _ σ _ _ O (λ a, sum.cases_on (f a) (g a) (λ c, some (h a c))) :=
...
theorem fix
  {O : ℕ → ℕ} {f : α →. σ ⊕ α} (hf : partrec O f) : partrec O (pfun.fix f) :=
let F : α → ℕ →. σ ⊕ α := λ a n,
  n.elim (some (sum.inr a)) $ λ y IH, IH.bind $ λ s,
  sum.cases_on s (λ _, roption.some s) f in
have hF : partrec₂ O F :=
...
have hp : partrec₂ O p := hF.map O ((sum_cases computable.id
